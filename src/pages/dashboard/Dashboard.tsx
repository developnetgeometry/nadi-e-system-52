  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from("announcements").select("*").eq("status", "active").lte("start_date", new Date().toISOString()).or(`end_date.gt.${new Date().toISOString()},end_date.is.null`).order("created_at", {
        ascending: false
      }).limit(5);
      if (error) {
  ...
  return <DashboardLayout>
      <ErrorBoundary FallbackComponent={ErrorFallback}>
        ...
        <Card className="mb-2">
          ...
          <CardContent>
            <AnnouncementCarousel announcements={announcements} loading={loading} />
          </CardContent>
        </Card>
        <DynamicDashboard />
      </ErrorBoundary>
