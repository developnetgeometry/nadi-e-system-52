...
export const AnnouncementAttachment: React.FC<AnnouncementAttachmentProps> = ({ attachments }) => {
        ...
        {attachments.map((file, index) => (
          <div key={index} className="flex items-center justify-between p-2 border rounded-md bg-muted/30">
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              {file.size && <span className="text-xs text-muted-foreground ml-2">({formatFileSize(file.size)})</span>}
            </div>
            <Button
              variant="outline"
              size="sm"
