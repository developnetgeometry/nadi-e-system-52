...
const RegisterProgrammeForm: React.FC = () => {
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      ...
      const startDateTime = `${format(data.startDate, "yyyy-MM-dd")}T${
        data.startTime
      }:00`;
      const endDateTime = data.endDate
        ? `${format(data.endDate, "yyyy-MM-dd")}T${data.endTime}:00`
        : `${format(data.startDate, "yyyy-MM-dd")}T${data.endTime}:00`;

      // Step 2: Insert into nd_event
      const { data: insertedEvent, error: eventError } = await supabase
        .from("nd_event")
        .insert({
          category_id: Number(data.category_id), // Ensure this field exists in the database schema
          subcategory_id: Number(data.subcategory_id),
          program_id: Number(data.program_id),
          module_id: Number(data.module_id),
          status_id: 1, // optional: adjust if needed
          site_id: null, // optional: adjust if needed
          target_participant: data.targetParticipant,
          program_mode: data.mode,
          program_method: data.eventType,
          location_event: data.location,
          start_datetime: startDateTime,
          end_datetime: endDateTime,
          duration: duration,
          program_name: data.programmeName,
          description: data.description,
          trainer_organization: data.trainerName,
          total_participant: Number(data.maxParticipants),
          total_new_member: 0, // optional: adjust if needed
          requester_id: null, // optional: set current user ID if available
          created_by: null, // optional: set current user ID if available
      ...
      // Step 3: Insert file attachments (if any)
      if (uploadedFiles.length > 0) {
        const attachments = uploadedFiles.map((filePath) => ({
          event_id: eventId,
          file_path: filePath,
          created_at: new Date().toISOString(),
          created_by: null, // optional: set current user ID if available
      ...
      toast({
        ...
        description: `${data.programmeName} has been saved.`,
      });
      ...
      toast({
        title: "Failed to register programme",
