    ...
    <Card className="mb-6">
      ...
      <CardContent className="space-y-4">
        {announcements.map(announcement => (
          <Alert key={announcement.id} className="bg-primary/5 border-primary/20">
            ...
            <AlertDescription className="mt-2">
              <div className="text-gray-700 dark:text-gray-300">{announcement.message}</div>
              <div className="mt-2 text-xs text-muted-foreground">
                Valid: {formatDate(announcement.start_date)} - {announcement.end_date ? formatDate(announcement.end_date) : "Indefinitely"}
              </div>
            </AlertDescription>
          </Alert>
        ))}
