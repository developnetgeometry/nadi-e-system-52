...
import { sendNotificationToUsers, sendTemplateNotificationToUsers, sendTestEmailWithTemplate, sendTestPushWithTemplate } from "@/utils/notification-utils";
...
export function NotificationUsageGuide() {
  ...
  async function handleTestTemplateToMultipleUsers() {
    ...
    try {
      ...
      console.error("Error sending template notifications to multiple users:", error);
      toast({
        title: "Error",
  ...
  { 
    ...
    date: new Date().toLocaleDateString()
              ...
              <Alert className="mt-4 bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800">
                <AlertDescription>
                  Note: In a production environment, you should implement proper email sending functionality 
                  with services like SendGrid, Resend, or Amazon SES.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
...
const result = await sendTestPushWithTemplate(
  ...
  { 
    ...
    date: new Date().toLocaleDateString()
              ...
              <Alert className="mt-4 bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800">
                <AlertDescription>
                  Note: In a production environment, you should implement proper push notification functionality 
                  with services like Firebase Cloud Messaging (FCM) or OneSignal.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
                      ...
                      {users.map(user => (
                        <div key={user.id} className="flex items-center space-x-2">
                          ...
                          <Label htmlFor={`user-${user.id}`} className="flex items-center space-x-2">
                            ...
                            <span>{user.full_name || user.email || `User ${user.id.substring(0, 8)}`}</span>
                          </Label>
                        </div>
                      ))}
                    </div>
