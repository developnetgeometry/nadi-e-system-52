...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (profileError || !profile || profile.user_type !== 'super_admin') {
      return new Response(JSON.stringify({ error: 'Unauthorized - requires super_admin privileges' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (storageError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to create storage URL for backup' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (requestError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to create backup request', details: requestError }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
